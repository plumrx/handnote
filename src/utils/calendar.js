 ...
 * @date 2014-7-21
 * @date 2016-8-13 Fixed 2033hex、Attribution Annals
 * @date 2016-9-25 Fixed lunar LeapMonth Param Bug
 * @date 2017-7-24 Fixed use getTerm Func Param Error.use solar year,NOT lunar year
 * @version 1.0.3
 * @example 公历转农历：calendar.solar2lunar(1987,11,01); //[you can ignore params of prefix 0]
...
export default {
   ...
   * 公历每个月份的天数普通表
   * @returns {number[]}
   * @example [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
   */
   ...
   * 天干地支之地支速查表
   * @returns {string[]}
   * @example ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥']
   */
   ...
   * 24节气速查表
   * @returns {string[]}
   * @example ['小寒', '大寒', '立春', '雨水', '惊蛰', '春分', '清明', '谷雨', '立夏', '小满', '芒种', '夏至', '小暑', '大暑', '立秋', '处暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪', '冬至']
   */
  solarTerm: ['小寒', '大寒', '立春', '雨水', '惊蛰', '春分', '清明', '谷雨', '立夏', '小满', '芒种', '夏至', '小暑', '大暑', '立秋', '处暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪', '冬至'],

  /**
   * 1900-2100 各年的 24 节气日期速查表
   * @returns {string[]} 0x string For splice
   */
  sTermInfo: [
   ...
   * 数字转中文速查表
   * @returns {string[]}
   * @example ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九', '日']
   */
  nStr1: ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九', '日'],
   ...
   * @returns {number}
   * @example var count = calendar.lYearDays(1987); // count=387
   */
  lYearDays (y) {
    var sum = 348
   ...
   * @param {number} y lunar Year
   * @returns {number} (0-12) 闰月所在月份
   * @example var leapMonth = calendar.leapMonth(1987); // leapMonth=6
   */
  leapMonth (y) {
   ...
   * @example var leapMonthDay = calendar.leapDays(1987); // leapMonthDay=29
   */
  leapDays (y) {
    if (this.leapMonth(y)) {
   ...
   * @example var MonthDay = calendar.monthDays(1987, 9); // MonthDay=29
   */
  monthDays (y, m) {
   ...
   * @example var solarDays = calendar.solarDays(1988, 2); // solarDays=29
   */
  solarDays (y, m) {
  ...
  /**
   * 公历月、日判断所属星座
   * @param {number} cMonth [description]
   * @param {number} cDay [description]
   * @returns {string}
   */
  toAstro (cMonth, cDay) {
   ...
   * @returns {number} day
   * @example var _24 = calendar.getTerm(1987,3); // _24=4;意即1987年2月4日立春
   */
  getTerm (y, n) {
  ...
  /**
   * 传入农历数字月份返回汉语通俗表示法
   * @param {nimber} m lunar month
   * @returns {string}
   * @example var cnMonth = calendar.toChinaMonth(12); // cnMonth='腊月'
   */
  toChinaMonth (m) {
    if (!m || m > 12 || m < 1) throw new RangeError('允许的值为 1-12')
   ...
   * @returns {string}
   * @example var cnDay = calendar.toChinaDay(21); // cnMonth='廿一'
   */
  toChinaDay (d) {
    var s
    switch (d) {
   ...
   * @param y year
   * @returns {string}
   * @example var animal = calendar.getAnimal(1987); // animal='兔'
   */
  getAnimal (y) {
  ...
  solar2lunar (y, m, d) { // 参数区间1900.1.31~2100.12.31
    if (y < 1900 || y > 2100) return -1 // undefined转换为数字变为NaN
  ...
  lunar2solar (y, m, d, isLeapMonth = false) { // 参数区间1900.1.31~2100.12.1
    ...
    // 计算农历的时间差
    var offset = 0
    for (var i = 1900; i < y; i++) offset += this.lYearDays(i)
  ...
  /**
   * 根据农历年份月份索引获得详细公历农历信息
   *
   * @author mutoe <mutoe@foxmail.com>
   * @param {number} y lunar year
   * @param {number} mIndex lunar month index (0-12) 含闰月
   * @param {number} d lunar day
   * @returns {Object}
   * @example
